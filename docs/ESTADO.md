...
| Tests unitarios | ✅ **270/270 verdes, 0 skips** | 18 suites — 252 tras Caso 4 Global; +11 con el wiring API de `extraeReferenciaPorTipo` (PR #18); +1 regresión de padding bpchar en `grouping.service.spec.ts` (PR #20, 2026-07-08); +6 con OC/HES como `<Referencia>` real en modo Global (PR #24, 2026-07-09) |
...
- [ ] **OPEN-2: ¿`extraeReferenciaPorTipo` debe partir la proforma por `guivaloragrupador`?** (observado 2026-07-08) — en el E2E de verificación del fix de recompute, `generar` produjo **una sola** proforma (`gfackey=133`) conteniendo ambas guías pese a que sus `guivaloragrupador` difieren (`555001` vs `777002`). El preview muestra las refs `801/555001`, `HES/777002` y los dos `52` ordenados dentro de una única factura (consistente con la regla de orden OC>HES>guías). Falta confirmar la intención de negocio: si el agrupador por OC/HES debe generar **una proforma por valor distinto** (2 facturas aquí) o **una sola** que consolide todas las referencias. Ajeno al bug de recompute (que ya está cerrado); depende de la lógica de agrupación de `FacturasService.generarProformas`, no de `GroupingService`.
