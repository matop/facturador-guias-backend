...
Última actualización: 2026-07-23 (cierre de sesión). PR #68 (Referencia Global vía `CODIGO REFERENCIA`) mergeada — hotfix de Enternet desplegado, emisión confirmada en QA (gfackey=189, folioSii=411248) sin cambios de código propios. PR #70 mergeada: `rutEmisor` deja de ser query param del operador en `POST .../proforma/generar` y `POST .../proforma` — se deriva del `<RUTEmisor>` del XML de cada guía por grupo/partición (aborta con 422 si es ambiguo o falta); corrige el 400 "rut es obligatorio" que veía facturaGdes al no tener ese dato ni UI para pedirlo. Server de dev en `:3334` reconstruido y reiniciado con el fix. `adr-0004-extraccion-parametros` (PR #60, draft) sigue intacta, sin definir mecanismo de extracción. Historial completo de sesiones anteriores (mayo–julio 2026)
...
| Tests unitarios | ✅ 292/292, 0 skips | 19 suites |
