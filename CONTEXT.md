...
- **Global**: sin bloque `<Referencia>` — guías, OCs y HES colapsan **todos juntos** como texto en `DESCRIPCION ADICIONAL` del Detalle (`"{folios guía} | OC: {folios OC} | HES: {folios HES}"`, segmentos vacíos omitidos). Aplica cuando el total > 40. Límite: 143 guías por proforma (límite de página en PDF del DTE).
...
⚠️ **Pendiente (fuera de alcance de esta sesión)**: wiring en `facturas.service.ts` — nadie llama todavía a `parseReferencias` por guía ni arma el array `referenciasExternas` que se le pasa a `buildMensaje`. Falta: (1) llamar `parseReferencias(rawXml)` por cada guía en `_cargarGuiasParaEmision`/`_emitir` (mismo punto donde ya se parsea el XML), (2) loguear las `descartadas` que devuelve, (3) concatenar las `referencias` de todas las guías (sin deduplicar — el dedup ya lo hace `buildMensaje`) y pasarlas en `MensajeInput.referenciasExternas`. Sin este paso, OC/HES nunca llegan al Mensaje V5 real aunque la lógica esté testeada y lista.
