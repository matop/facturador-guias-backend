...
- **Global (overflow)**: se activa cuando el total de referencias (guías individuales + OC deduplicadas + HES deduplicadas) supera el límite MAX de 40, sin importar el Modo de Detalle configurado. Texto propio, distinto a S.G.: `"Segun Guias: {folio1} {folio2} ..."` — folios completos, separados por espacio, sin abreviar y sin prefijo `f`. Las `<Referencia>` usan una referencia global en vez de guías individuales.
  - ✅ **Implementado 2026-07-02** en `mensaje-builder.ts` (`MAX_REFERENCIAS_INDIVIDUALES = 40`) — al 2026-07-02 `isGlobal` solo contaba guías, sin OC/HES (esas referencias no existían todavía en el pipeline). **Diseño de la interacción con OC/HES resuelto en sesión 2026-07-06 e implementado en sesión 2026-07-06** (TDD, ver `docs/PRD-referencias-oc-hes.md`): `isGlobal = (guias.length + oc.length + hes.length) > 40` (OC/HES ya deduplicadas por `(tipo, folio)`). Cuando Global se activa, OC/HES **colapsan junto con las guías** en el mismo campo `DESCRIPCION ADICIONAL` — no se listan aparte ni se ignoran. Formato: `"{folios guía} | OC: {folios OC} | HES: {folios HES}"`, omitiendo el segmento `OC:`/`HES:` por completo si no hay ninguna de ese tipo. Se eligió este camino (en vez de listar OC/HES aparte en líneas `5:|` individuales) para no tocar el mecanismo de Referencia ya confirmado en QA (ver abajo, sin bloque `<Referencia>` en Global).
  ...
  - Referencia: el mecanismo **confirmado y funcionando contra Enternet QA** (emisión real, folioSii=411211) es omitir el bloque `<Referencia>` por completo — sin líneas `4:|`/`5:|`, sin campos `TIPO/FOLIO/ACCION REFERENCIA` en el encabezado. Los folios viajan solo en `DESCRIPCION ADICIONAL`.
  - ⚠️ **Bloque EXPERIMENTAL activo en el código, sin revertir**: `mensaje-builder.ts` reintroduce (a pedido explícito del usuario, sesión 2026-07-03) los campos `TIPO/FOLIO/ACCION REFERENCIA` + línea `5:|52|0|{fecha}` al final de `buildMensaje`, para retestear si Enternet corrige un bug confirmado de su parser (genera dos bloques `<Referencia>` contradictorios desde un solo Mensaje V5 — ver `docs/consulta-enternet-referencia-global.md`). **Al 2026-07-06 Enternet seguía sin confirmar el fix** — no reintentar variantes propias, solo retomar cuando avisen. No revertir este bloque hasta entonces.
...
- **Individual**: una `<Referencia>` por guía (tipo 52) + OCs deduplicadas (tipo 801) + HES deduplicadas, cada una en su propia línea `5:|`. Aplica cuando el total ≤ 40.
- **Global**: las **guías** colapsan en UNA referencia global (`5:|52|0|{fecha}`, `IndGlobal=1`) — no se listan una por una (superan el tope SII de 40 `<Referencia>`/DTE). Las **OCs (801) y HES** deduplicadas SÍ se emiten como `<Referencia>` reales (`5:|801|…`/`5:|HES|…`), en orden OC > HES > referencia global, con `RAZON REFERENCIA` como 4to campo (todas las líneas `5:|` del bloque lo declaran, vacío para la global — regla de consistencia de Enternet, [ParseErr001]). `DESCRIPCION ADICIONAL` del Detalle lleva **solo** los folios de guía (ya NO las OC/HES — ver `docs/PLAN-referencias-oc-hes-en-global.md`, D1-A, PR #23). Aplica cuando el total > 40. Límite: 143 guías por proforma (límite de página en PDF del DTE). Nota: Enternet arma la referencia global desde el trío del encabezado `1:|TIPO/FOLIO/ACCION REFERENCIA`, que en el XML final queda **primera** (antes de OC/HES) — quirk de su parser, no controlable desde el Mensaje. Historial en [enternet-v5](docs/consulta-enternet-referencia-global.md).
...
✅ **Implementado 2026-07-06 (TDD)**: `parseReferencias(xml)` en `xml-parser.utils.ts` (+ expuesta en `XmlParserService`) y su integración en `mensaje-builder.ts` (`MensajeInput.referenciasExternas`, dedup interno por `(tipo, folio)`, líneas `5:|801|.../5:|HES|...` en modo individual, colapso en `DESCRIPCION ADICIONAL` en modo Global). Tests: `xml-parser.service.spec.ts` (describe `parseReferencias`), `mensaje-builder.spec.ts` (describe `buildMensaje — Referencias Externas (OC/HES)`), `mensaje-builder-referencias-global.spec.ts` (archivo aparte para la interacción con el umbral 40, según lo definido en el PRD). Builder de test nuevo: `src/xml/xml-test-builders.ts` (`buildGuiaXml`).
✅ **Wireado en `facturas.service.ts` (sesión 2026-07-06)**: `_emitir`/`previewMensaje` fetchean el XML de todas las guías (no solo la primera) para extraer referencias vía `parseReferencias` y pasarlas a `buildMensaje` como `referenciasExternas`. Las descartadas se loguean sin bloquear la emisión. **Emisión real confirmada en QA** (folioSii=411212/411213/411214) — ver `docs/ESTADO.md`.

### Manejo de errores de `parseReferencias` (XML de entrada malformado)
Decidido en sesión de grill 2026-07-06 (ver `docs/PRD-referencias-oc-hes.md`), dos casos distintos con tratamiento distinto:
- **`<TpoDocRef>` no reconocido** (ni `52`, ni `801`, ni `HES` — incluye tipos SII válidos mas fuera de alcance, ej. otra guía referenciada por `52`, y también cualquier código realmente desconocido): se **ignora, no bloquea**, pero se reporta. `parseReferencias` sigue siendo una función pura (sin side effects, mismo criterio que el resto de `xml-parser.utils.ts`) — devuelve también las ocurrencias descartadas junto al resultado, y es el **caller** (con acceso a `Logger` de Nest) quien decide loguear. No hay caso especial para `52`: se trata igual que cualquier otro tipo no-OC/HES.
- **`801`/`HES` reconocido pero con `<FolioRef>` o `<FchRef>` faltante** (dato incompleto en un tipo que sí nos interesa): se **propaga un error (throw)**, bloqueando la emisión — a diferencia del resto de la política MVP tolerante (multiplicidad, `CODIGO` inconsistente), acá se prefiere frenar y forzar revisión manual del XML de origen antes de facturar una referencia incompleta.
