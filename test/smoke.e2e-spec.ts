...
interface ReglaEmpresaRespuesta {
  reglaidl: string;
  empkey: string;
}
...
interface ItemAgrupadoRespuesta {
  cliente: { rut: string; nombre: string };
  grupos: GrupoRespuesta[];
}

interface ProformaRespuesta {
  id: string;
  folio: string;
  cliente: { rut: string; nombre: string };
...
describe('Smoke E2E — empkey=977', () => {
  ...
  it('cada cliente tiene rut, nombre, cantidadGuias, montoTotal, reglaIdl', async () => {
    ...
    const body = res.body as ClienteRespuesta[];
    if (body.length === 0) {
  ...
  it('GET /reglas/empresa/:empkey devuelve array de reglas', async () => {
    ...
    expect(Array.isArray(res.body)).toBe(true);
    const body = res.body as ReglaEmpresaRespuesta[];
    if (body.length > 0) {
      const r = body[0];
  ...
  it('PUT /empresas/:empkey/clientes/:rut/regla → 200', async () => {
    ...
    await request(app.getHttpServer())
      .put(
        `/empresas/${EMPKEY}/clientes/${encodeURIComponent(primerRut)}/regla`,
      )
  ...
  it('clientes refrescan reglaIdl tras el PUT', async () => {
    ...
    const cliente = (res.body as ClienteRespuesta[]).find(
      (c) => c.rut === primerRut,
    );
    if (cliente) {
  ...
  it('cada item tiene cliente + grupos[], cada grupo tiene valorAgrupador/reglaIdl/cantidadGuias/montoTotal/folios', async () => {
    ...
    const body = res.body as ItemAgrupadoRespuesta[];
    if (body.length === 0) {
    ...
    if (item.grupos.length > 0) {
      ...
      expect(
        grupo.reglaIdl === null || typeof grupo.reglaIdl === 'string',
      ).toBe(true);
    }
  });
  ...
  it('cada proforma tiene id, folio, cliente, regla, estado, montoTotal, fecha, valorAgrupador', async () => {
    ...
    const body = res.body as ProformaRespuesta[];
    if (body.length === 0) {
      console.warn(
        '⚠ Sin proformas para periodo',
        PERIODO,
        '— shape test omitido',
      );
      return;
  ...
  it('POST /empresas/:empkey/facturas/proforma/generar devuelve { created, skipped }', async () => {
    ...
    const body = res.body as GenerarRespuesta;
    expect(body).toHaveProperty('created');
    expect(body).toHaveProperty('skipped');
    expect(typeof body.created).toBe('number');
    expect(typeof body.skipped).toBe('number');
    console.log(`  generar: created=${body.created} skipped=${body.skipped}`);
  });
  ...
  it('proformas BORRADOR disponibles tras generar', async () => {
    ...
    const body = res.body as ProformaRespuesta[];
    if (body.length > 0) {
      primerProformaId = body[0].id;
      console.log(`  primerProformaId=${primerProformaId}`);
    } else {
      console.warn(
        '⚠ Sin proformas BORRADOR — tests de aprobar/anular omitidos',
      );
    }
  });
  ...
  it('PATCH .../aprobar cambia estado a APROBADA', async () => {
    ...
    const res = await request(app.getHttpServer())
      .patch(
        `/empresas/${EMPKEY}/facturas/proforma/${primerProformaId}/aprobar`,
      )
      .expect(200);
