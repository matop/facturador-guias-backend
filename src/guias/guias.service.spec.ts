...
const mockGroupingService = { batchComputeAgrupadores: jest.fn().mockResolvedValue(new Map()) };

// Manager capturado por cada test que ejecuta la transacción
let mockManager: { save: jest.Mock; update: jest.Mock; createQueryBuilder: jest.Mock };
...
const mockDataSource = {
  transaction: jest.fn().mockImplementation(async (cb: (m: typeof mockManager) => Promise<void>) => {
    ...
    mockManager = {
      save: jest.fn().mockImplementation(async (_EntityClass: unknown, entities: unknown) => entities),
...
const makeRow = (folio: string, gclirut: string, filePath = 'http://xml/dte.xml') => ({
  'Folio': folio,
  'Codigo Tipo': '52',
...
const PARSED_DOC = {
  rawXml: '<DTE></DTE>',
  emisor: { rutEmisor: '11111111-1', razonSocial: 'Emisor SA', giro: 'Giro', telefono: '', acteco: '' },
  receptor: {
...
describe('GuiasService — syncFromReporte orchestration', () => {
  ...
  let mockRepo: { save: jest.Mock; update: jest.Mock; find: jest.Mock; findOne: jest.Mock };
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(GuiaImpuesto), useValue: mockImpuestoRepo },
  ...
  it('retorna synced:0 y no hace fetches cuando no hay rows', async () => {
    ...
    const result = await service.syncFromReporte('1', '11111111-1', '2026-01-01', '2026-01-31');
  ...
  it('hace un fetch por cada cliente único — en paralelo dentro de cada chunk', async () => {
    ...
    await service.syncFromReporte('1', '11111111-1', '2026-01-01', '2026-01-31');

    expect(mockXmlParser.fetchDocument).toHaveBeenCalledTimes(2);
    expect(mockXmlParser.fetchDocument).toHaveBeenCalledWith('http://xml/100.xml');
    expect(mockXmlParser.fetchDocument).toHaveBeenCalledWith('http://xml/101.xml');
  });
  ...
  it('deduplica clientes — no hace fetch duplicado para el mismo RUT', async () => {
    ...
    mockBackofficeAdapter.getGuias.mockResolvedValueOnce(rows);
    mockClientesService.findOrCreate.mockResolvedValueOnce({ cliente: {}, created: true });
  ...
  it('incluye synced y clientesCreated en el resultado', async () => {
    mockBackofficeAdapter.getGuias.mockResolvedValueOnce([makeRow('100', '22222222-2')]);
    mockClientesService.findOrCreate.mockResolvedValueOnce({ cliente: {}, created: true });
  ...
  it('ejecuta Fase 2 dentro de una transacción (todo o nada)', async () => {
    mockBackofficeAdapter.getGuias.mockResolvedValueOnce([makeRow('100', '22222222-2')]);
    mockClientesService.findOrCreate.mockResolvedValueOnce({ cliente: {}, created: false });
  ...
  it('guarda guías y un GuiaImpuesto (IVA cod=14) vía el manager de transacción', async () => {
    ...
    // Primer save: Guia[]
    expect(mockManager.save).toHaveBeenCalledWith(Guia, expect.arrayContaining([
    ...
    const qb = mockManager.createQueryBuilder.mock.results[0].value as {
      into: jest.Mock; values: jest.Mock;
    };
    expect(qb.into).toHaveBeenCalledWith(GuiaImpuesto);
    expect(qb.values).toHaveBeenCalledWith(expect.arrayContaining([
