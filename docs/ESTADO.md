...
Última actualización: 2026-07-23 (cierre de sesión). Commit `0373c9e` (directo a `main`, sin PR): se expone `valorAgrupador` en el listado de proformas (`GET .../facturas/proforma` y `buildProformaDto`) — el frontend necesitaba matchear cada proforma con su grupo en `/guias/agrupadas` y no había forma de correlacionarlos. Se deriva de `gde.guia.guivaloragrupador` vía `MIN()` sobre el join existente, sin migración de esquema, normalizando `null` a `'_sin_regla'`. 293/293 tests verdes. Server de dev en `:3334` reiniciado (el proceso corriendo era previo al commit) y verificado en vivo contra QA real (empkey=977, ej. `valorAgrupador:"LAMPA"`). PR #68 (Referencia Global vía `CODIGO REFERENCIA`) y PR #70 (`rutEmisor` derivado del XML) siguen mergeadas de sesiones anteriores. `adr-0004-extraccion-parametros` (PR #60, draft) sigue intacta, sin definir mecanismo de extracción. Historial completo de sesiones anteriores (mayo–julio 2026)
...
| Proforma (`factura` + `facturaguias`) | ✅ Funcional | partición `(gclirut, guireglaidl, guivaloragrupador)` — 1 proforma por valor de agrupador; chunking vía `parametrosService.getMaximoGuias(empkey)` (antes constante `MAX_GUIAS_POR_FACTURA=40`, ver fila `ParametrosModule` abajo); estados BORRADOR→APROBADA→EMITIDA\|FALLIDA\|ANULADA; `rutEmisor` ya no es input del operador — se deriva del `<RUTEmisor>` del XML de cada guía por partición, 422 si es ambiguo (PR #70); listado expone `valorAgrupador` (MIN sobre join, `'_sin_regla'` si null) para que el frontend matchee proforma↔grupo (commit `0373c9e`) |
...
| Tests unitarios | ✅ 293/293, 0 skips | 19 suites |
