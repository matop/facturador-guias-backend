...
**Resultado (2026-07-13):** EJECUTADA, con desvío significativo del plan original — el pre-flight (paso 1 de la ejecución) invalidó la premisa de la investigación previa:

- Las "327 guías reales de 2026-03" de `76407930-2` **no eran reales**: `guitipo` 994/995/999, `guifilepath` con XML inline `data:text/xml;base64,...` de contenido genérico ("Av Test 123") — debris sintético de una sesión anterior mal etiquetado como real.
- El único volumen real (`guitipo=52`) de este cliente bajo `empkey=1163` está en **2026-06** (410 guías), y estaba 100% consumido: 70 guías ya en facturas `EMITIDA` (gfackey 24/36/37/38/86) y 340 en 17 proformas `BORRADOR` huérfanas (gfackey 87-96, 133-134, 141, 146-147, 150-151) — más proformas huérfanas de las que la investigación previa había detectado (17, no 10).
- **Hallazgo colateral (no corregido, requiere seguimiento):** 40 de esas 410 guías (folios 525104-525113 y otros) estaban simultáneamente linkeadas a **dos** facturas `EMITIDA` distintas (gfackey 36 y 86) — posible doble facturación real. Ver `gde.facturaguias` para el detalle completo. Queda como ítem abierto para Sesión 5 o un issue dedicado.
- Con autorización explícita del usuario, se limpiaron las 17 proformas `BORRADOR` vía `POST /facturas/proforma/limpiar?periodo=2026-07` (17 anuladas), liberando 340 guías reales de junio para el tracer.
- **Bug real encontrado y corregido:** `POST /guias/recomputar` (bulk, `EmpresasService.recomputarTodasLasGuias`) agrupaba las guías por `g.gclirut` sin recortar el padding de `character(20)` de Postgres, y pasaba ese valor con padding a `GroupingService.computeAgrupador`, cuyo lookup de cliente usa claves recortadas → miss silencioso → `guireglaidl`/`guivaloragrupador` quedaban en `NULL` para el 100% de las guías del período, sin contarlo como error (`{actualizados:0, errores:0}` con éxito HTTP 200). Reproducido en vivo: el recompute narrow (`PUT /clientes/:rut/regla`) dejó las 410 guías en `por_comuna`/`Providencia` correctamente; la siguiente llamada al endpoint bulk las volvió a `NULL` sin avisar. Fix: recortar `g.gclirut.trim()` antes de agrupar (`src/empresas/empresas.service.ts`), con test de regresión en `empresas.service.spec.ts` (`describe('recomputarTodasLasGuias', ...)`, 2 casos nuevos). Confirmado en vivo tras el fix: `{"procesados":410,"actualizados":410,"errores":0}`, idéntico al resultado del narrow recompute.
- `POST /facturas/proforma/generar?periodo=2026-06&rut=968880004` sobre las 340 guías libres → **11 proformas BORRADOR** (chunking `MAX_GUIAS_POR_FACTURA=40`: 10×40 + 1×10), todas `reglaidl='por_comuna'`.
- `PATCH /aprobar` sobre 1 sola (gfackey=166, 10 guías) → **`estado='EMITIDA'`, `folioSii=411236`**, confirmando que la credencial de emisión (`968880004` / `RutUsuario=16714595-7`) sigue vigente.
- Cierre: se anularon las 10 proformas BORRADOR restantes del tracer (`limpiar?periodo=2026-07`) para no dejar debris nuevo, dejando solo el `folioSii=411236` como resultado.
- Punto "3-7 guías de fixture" del objetivo: cerrado — el tracer completo corrió sobre 410 guías reales de backoffice, no fixture sintético.
...
- [x] Sesión 4 — `assignRegla`/recompute a volumen real — **EJECUTADA 2026-07-13** (folioSii=411236, gfackey=166, 410 guías reales de 2026-06). Encontró y corrigió un bug real: `POST /guias/recomputar` bulk wipeaba `guireglaidl`/`guivaloragrupador` a `NULL` en el 100% de las guías por un mismo padding de RUT (`character(20)`) no recortado (ver detalle en la sección Sesión 4 arriba). También limpió 17 proformas BORRADOR huérfanas de junio (más de las 10 detectadas en la investigación previa) y dejó abierto un hallazgo de doble-facturación (40 guías linkeadas a 2 facturas EMITIDA simultáneas) para Sesión 5.
