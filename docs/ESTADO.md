...
Última actualización: 2026-07-21 (segundo cierre de sesión del día — worktree `phase-b-reglaidl-casing` tenía un commit huérfano (`265efbb`, sin PR propia) que fija el contrato de `PUT /empresas/:empkey/clientes/:rut/regla`: el DTO con `forbidNonWhitelisted` ya rechaza `reglaidl` en minúscula con 400 — el bug real reportado estaba en el frontend `facturaGdes`, no en este backend. Verificado con `pnpm test:e2e --testPathPatterns=smoke` (15/16 verde; el único fallo, `POST .../generar`, es preexistente en `main` — datos de periodo, no lógica) y mergeado como [PR #66](https://github.com/matop/facturador-guias-backend/pull/66). Checkout principal sincronizado a `origin/main` (298f4c5). `adr-0004-extraccion-parametros` (PR #60, draft) sigue intacta, abierta intencionalmente a la espera de confirmar con el jefe el mecanismo concreto de extracción de `src/parametros/`, ver memoria `plan-contexto-genexus`. Historial completo de sesiones anteriores (mayo–julio 2026)
